...
{
	return rcp(lerp(LdotH * LdotH, 1, alpha * alpha * 0.5f));
}
...
// Diffuse irradiance - Skybox
float3 Diffuse_IBL(in float3 N, in TextureCube skybox, SamplerState smp)
{
	return skybox.Sample(smp, N).xyz;
}
//
...
float3 Specular_IBL(in float3 N, in float3 V, in float lodBias, in TextureCube skybox, SamplerState smp)
{
	float mip = lodBias * 4;
	float3 dir = reflect(-V, N);
	return skybox.Sample(smp, dir, mip).xyz;
}
...
float4 LightSurface(
	...
	in int numLights, in float4 lightColor, in float4 lightDirection,
	in float4 albedo, in float roughness, in float metallic, in float ambientOcclusion,
	in TextureCube skybox, SamplerState smp, in TextureCube skybox_irr)
{
	...
	// Blend base colors
	//const float3 c_diff = lerp(albedo.xyz, albedo.xyz * 0.2f, metallic) *ambientOcclusion;
	...
	// Directional light
	acc_color.xyz += NdotL * lightColor * ((c_diff * diffuse_factor) + specular);

	// Add diffuse irradiance
	float3 diffuse_env = Diffuse_IBL(N, skybox_irr, smp);
	acc_color.xyz += c_diff * diffuse_env;

	//specular radiance
	float3 specular_env = Specular_IBL(N, V, roughness, skybox, smp);
	acc_color.xyz += c_spec * specular_env;
	acc_color.a = albedo.a;

	//return float4(diffuse_env.xyz * specular_env.xyz, 1.0f);

	if (g_ChView.AmbientOcclusion == true)
	{
		acc_color = float4(ambientOcclusion, 0, 0, albedo.a);
	}
	if (g_ChView.Roughness == true)
	{
		acc_color = float4(0, roughness, 0, albedo.a);
	}
	if (g_ChView.Metalic == true)
	{
		acc_color = float4(0, 0, metallic, albedo.a);
	}
	if (g_ChView.ShadowMap == true)
	{
		acc_color = float4(specular, 0) + float4(ambientOcclusion, roughness, metallic, albedo.a);
	}

	if (g_ChView.IBL_SkyBox == true)
	{
		acc_color.xyz += diffuse_env;
		acc_color.xyz += specular_env;
		acc_color.a = albedo.a;
	}
