	...
	float2 uv : TEXCOORD)
{
	...
	pos = mul(pos, mTM);
	pos = mul(pos, mWVP); //world로 변환
	//pos = mul(pos, mProj); //world로 변환
	//pos = mul(pos, mWVP);
...
Texture2D texShadowMap : register(t6);     //레지스터 직접 지정. 
TextureCube texSkyBox_irr : register(t7);     //레지스터 직접 지정. 
...
) : SV_TARGET
{
	...
	if (g_Mtrl.Texon == true)
	{
		...
		{
			EmissiveMap = texEmissive.Sample(smpLinear, uv);		// Emissive 색 얻어오기
		}
	}
